...
Working name: **pith**. Rename is cheap while this GitHub repo is private.
Locked: 2026-09-02.
...
Pith takes the same *ownership* idea (you keep the files) and a narrower
...
Pith is the product, so the map is:

| Repo | Job | Public? | Today |
|---|---|---|---|
| **Product** (this repo) | Engine, MCP, schema, docs, demo brain | Private now. Public later. | `k-hanafi/pith` |
...
point pith at that folder. Do not freeze the vault while building a
...
| Vector | Which pages are *about* this even if they used different words? | **Optional, v1.5+.** Fail-open: no API key means pith still works. |
...
that folder the same week they land. If pith cannot serve the vault without
...
GOAL: After v1, a coding agent connected to pith can answer a question from a
...
`pith check` on `examples/demo-brain` and get a clean pass without API keys.
