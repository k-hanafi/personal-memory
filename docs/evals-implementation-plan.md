...
|---|---|---|---|---|
| 0 | `eval-corpus` | `evals/brain/` (39 notes), `evals/deny-list.txt`, `tests/test_eval_brain_guard.py`, `docs/eval-corpus-plan.md`, spec and AGENTS pointers | none | in flight |
| 1 | `factory-setup` | `.github/workflows/test.yml` running pytest; this plan; `evals/runs/` in `.gitignore` | none | planned |
| 2 | `eval-loader-adapters` | `src/personal_memory/evals/`: `Case` and fixture loader, `Hit`, check functions, `recall` adapter, grep adapter in pure Python; unit tests | 0, 1 | planned |
| 3 | `eval-runner-cli` | runner, receipt writer, table report, `personal-memory eval run` with `--corpus --fixtures --adapter --family --out`; determinism test | 2 | planned |
| 4 | `eval-fixtures` | five TOML files (about 30 cases) from the corpus plan's planted problems; fixture validation test; `expect_confidence` added to spec | 2 (start), 3 (merge) | planned |
